...
class _Program(object):
    ...
    def __init__(self,
        ...
        if self.program is not None:
            ...
            if given_prefix:
                self.program=self.build_given_program(given_prefix)
            else:
    ...
    def build_given_program(self, prefix):
        """Build a naive random program.
        ...
        def is_variable(s):
            if s.startswith('x'):
                ...
                return False
        def variable_to_int(s):
            return int(s.split('_')[1])

        for s in prefix.split():
            if s in _function_map:
                function = _function_map[s]
                program.append(function)
            else:
                # We need a terminal, add a variable or constant
                if is_constant(s):
                    program.append(float(s))
                elif is_variable(s):
                    # We have to translate x_1 to int(1), x_2 to int(2) and so on
                    program.append(variable_to_int(s))
        return program
            ...
            for node in self.program:
                ...
                while len(apply_stack[-1]) == apply_stack[-1][0].arity + 1:
                    ...
                    if len(apply_stack) != 1:
                        ...
                        if not np.all(np.isfinite(intermediate_result)):
                            raise Exception(f"In calculation an non float occur ")
                        return intermediate_result

            # We should never get here
            return None
        except Exception as e:
            return np.full((X.shape[0]), 99999999999999999999999)
